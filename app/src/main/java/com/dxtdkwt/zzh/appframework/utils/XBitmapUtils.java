...
public class XBitmapUtils {
     ...
     * @return
     */
    public static String bitmapToBase64Blow500k(Bitmap bitmap) {
        String result = null;
        ByteArrayOutputStream baos = null;
        try {
            if (bitmap != null) {
                ...
                while ((bytes / 1024 > 500) && (options >= 10)) { //循环判断如果压缩后图片是否大于500kb,大于继续压缩
...
//                baos.close();
     ...
     * @return
     */
    public static String bitmapToBase64Blow2M(Bitmap bitmap) {
        String result = null;
        ByteArrayOutputStream baos = null;
        try {
            if (bitmap != null) {
                ...
                int options = 90;
                int bytes = baos.toByteArray().length;
                while ((bytes / (1024 * 1024) > 2) && (options >= 10)) { //循环判断如果压缩后图片是否大于500kb,大于继续压缩
                    baos.reset();//重置baos即清空baos
                    options -= 10;//每次都减少10
                    //第一个参数 ：图片格式 ，第二个参数： 图片质量，100为最高，0为最差  ，第三个参数：保存压缩后的数据的流
                    bitmap.compress(Bitmap.CompressFormat.JPEG, options, baos);//这里压缩options%，把压缩后的数据存放到baos中
...
//                baos.close();
     ...
     * @return 获取图像的Bitmap
     */
    public static synchronized Bitmap getBitmapFromUri(Uri uri, String imgPath, Activity mContext) {
        Bitmap bitmap = null;
        Bitmap bitmapOrigin = null;
        try {
            bitmapOrigin = getBitmapFormUri(mContext, uri);
            bitmap = rotaingImageView(readPictureDegree(imgPath), bitmapOrigin);
            return bitmap;
        } catch (Exception e) {
            e.printStackTrace();
            return null;
     ...
     * @return 获取图像的Bitmap
     */
    public static Bitmap getBitmapFromVideoUri(Uri uri, Context mContext) {
        try {
            if (uri != null) {
                if ("content".equals(uri.getScheme().toString())) {
    ...
    public static Bitmap getBitmapFromView2(View view) {
        ...
        if (cacheBitmap == null) {
            LogUtil.e("error","failed getViewBitmap(" + view + ") -->" +
