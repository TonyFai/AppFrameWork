...
import com.dxtdkwt.zzh.utilslibrary.ext.ScreenUtils;
...
//            Log.i(TAG, "onMeasure: childHeight= " + childHeight);
            ...
            if (lineWidth + childWidth > widthSize) {//换行
                if (lineViews.size() == 1 && lineViews.get(0).getLayoutParams().height == LayoutParams.MATCH_PARENT) {//如果一行只有一个元素
                    lineHeight = ScreenUtils.dp2px(150);
                }
                views.add(lineViews);
...
//            Log.i(TAG, "remeasureChild: " + lineHeight);
    ...
    public static class LayoutParams extends MarginLayoutParams {
        ...
        public LayoutParams(Context c, AttributeSet attrs) {
            ...
            TypedArray a = c.obtainStyledAttributes(attrs, R.styleable.FlowLayoutView_Layout);
            try {
                gravity = a.getInt(R.styleable.FlowLayoutView_Layout_android_layout_gravity, -1);
            } finally {
                a.recycle();
            }
...
//     * 2.测量：在onMeasure MeasureSpec.AT_MOST/EXACTIY
//     * 3.布局：onLayout 方法里面根据自己规则来确定Children的位置
//     * 4.绘制：onDraw
//     * 5.处理LayoutParams
//     * 6.触摸反馈：滑动事件
//     *
...
//        init();
//
//        //遍历所有的子View控件   对子View进行测量，分配具体行
//        int childCount = getChildCount();
//        for (int i = 0; i < childCount; i++) {
//            View child = getChildAt(i);
//            //进行测量 获取到当前子View的测量的宽度/高度
//            measureChild(child, widthMeasureSpec, heightMeasureSpec);
//            int childWidth = child.getMeasuredWidth();
//            int childHeight = child.getMeasuredHeight();
//
...
//            //如果放不下，换行 保存当前行的所有子View，累加行高，当前宽度，高度，置零
//            if (currWidth + childWidth > widthSize) {
//                //如果一行只有一个元素
//                if (lineViews.size() == 1 && lineViews.get(0).getLayoutParams().height == LayoutParams.MATCH_PARENT) {
//                    currHeight = ScreenUtils.dp2px(150);
//                }//换行
//                views.add(lineViews);
//                //创建新的一行
//                lineViews = new ArrayList<>();
////                lineViews.clear(); 不可使用 add 添加的是引用
//                flowLayoutWidth = Math.max(flowLayoutWidth, currWidth);
...
//                //滑动的值可能为负数  固然使用abs
//                //表示本身需要拦截处理
//                intercept = Math.abs(dy) > Math.abs(dx) && Math.abs(dy) > mTouchSlop;//表示本身需要拦截处理
//                break;
//            default:
//                break;
//        }
...
//        public LayoutParams(Context c, AttributeSet attrs) {
//            super(c, attrs);
//            TypedArray type = c.obtainStyledAttributes(attrs, R.styleable.FlowLayoutView_Layout);
//            try {
//                gravity = type.getInt(R.styleable.FlowLayoutView_Layout_android_layout_gravity, -1);
//            } finally {
//                //避免内存泄漏
//                type.recycle();
//            }
