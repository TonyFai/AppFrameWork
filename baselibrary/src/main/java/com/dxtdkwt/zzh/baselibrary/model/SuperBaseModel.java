...
import com.dxtdkwt.zzh.baselibrary.preference.BasicDataPreferenceUtil;
import com.dxtdkwt.zzh.baselibrary.utils.GsonUtils;
...
public abstract class SuperBaseModel<T> {
    ...
    public void unRegister(IBaseModelListener listener) {
        ...
        synchronized (this) {
            for (WeakReference<IBaseModelListener> weakReference : mWeakListenerArrayList) {
                IBaseModelListener listenerItem = weakReference.get();
    ...
    /**
     * 由于渠道处在App的首页，为了保证app打开的说话由于网络慢或者异常的情况下tablayout不为空，
     * 所以app对渠道数据进行了预制；
    ...
    /**
     * 该model 的数据是否需要缓存，如果需要请返回缓存的key
     *
     * @return
     */
    ...
    public void getCachedDataAndLoad() {
        if (getCachedPreferenceKey() != null) {
            ...
            if (getApkString() != null) {
                notifyCachedData(GsonUtils.<T>fromLocalJson(getApkString(), getTClass()));
            }
        }
        load();
    }
